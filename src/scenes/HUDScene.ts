...
export class HUDScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.restProgressBg = this.add.circle(width / 2, height / 2, 18, 0x000000, 0.5).setVisible(false);
    ...
    this.restingLabel = this.add
      .text(width / 2, height * 0.35, "Resting... (press any key to wake)", {
        fontSize: "12px",
        color: "#aaddff",
        stroke: "#000000",
        strokeThickness: 2,
  ...
  private createPauseMenu(width: number, height: number): Phaser.GameObjects.Container {
    ...
    const container = this.add.container(width / 2, height / 2, [overlay, title, saveBtn, resumeBtn, quitBtn]);
    return container;
  }
