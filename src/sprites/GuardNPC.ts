...
export class GuardNPC extends Phaser.Physics.Arcade.Sprite {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.nameLabel = scene.add.text(x, y - 24, 'Guard', {
      fontSize: '8px',
      color: '#DD2222',
      stroke: '#000000',
      strokeThickness: 2,
  ...
  private createAnimations(scene: Phaser.Scene): void {
    ...
    scene.anims.create({ key: `${SPRITE_KEY}-walk-down`, frames: scene.anims.generateFrameNumbers(SPRITE_KEY, row(0)), frameRate: 6, repeat: -1 })
    scene.anims.create({ key: `${SPRITE_KEY}-walk-left`, frames: scene.anims.generateFrameNumbers(SPRITE_KEY, row(1)), frameRate: 6, repeat: -1 })
    scene.anims.create({ key: `${SPRITE_KEY}-walk-right`, frames: scene.anims.generateFrameNumbers(SPRITE_KEY, row(2)), frameRate: 6, repeat: -1 })
    scene.anims.create({ key: `${SPRITE_KEY}-walk-up`, frames: scene.anims.generateFrameNumbers(SPRITE_KEY, row(3)), frameRate: 6, repeat: -1 })
    scene.anims.create({ key: `${SPRITE_KEY}-idle`, frames: scene.anims.generateFrameNumbers(SPRITE_KEY, row(4, 3)), frameRate: 3, repeat: -1 })
  }
