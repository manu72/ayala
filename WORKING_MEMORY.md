...
- **GameScene** (`src/scenes/GameScene.ts`): Main game loop, NPC management, input, chapters (~2200 lines after the Commit A–D system refactor; orchestrates Phaser lifecycle + input + camera + `update()` call order, delegating Camille, snatchers, colony dynamics, human presence, collapse, cat dialogue, and territory exploration into dedicated systems)
...
| `TerritorySystem.ts` | Territory claiming + benefits at The Shops; owns the flood-filled reachable-cell set + exploration-score lookup |
| `SnatcherSystem.ts`  | Nightly snatcher spawn/detect/capture lifecycle + colony-cat grab sweep; still re-exports `resolveSnatcherSpawnAction` for tests |
| `ColonyDynamicsSystem.ts` | Dumping events, background colony roster spawn, and the `StoryKeys.COLONY_COUNT` dynamic total (seed / bump / decrement); single mutation owner |
| `CamilleEncounterSystem.ts` | Camille beats 1–5 state machine, paired narrator/spoken surfaces, Beat-5 consent gate + pickup tween, Manu/Kish care routes |
| `HumanPresenceSystem.ts` | Ambient human spawn/update, stationary-greet cap + anchor, scripted/AI greeting bubbles, single `renderHumanBubble` surface, `register/unregister` API for Camille + Snatcher entities |
| `CollapseSystem.ts`  | Collapse rising/falling-edge detection, 3 s blackout pin, pre-teleport witness snapshot, witness-aware recovery narration |
| `CatDialogueController.ts` | Cat-dialogue engagement gating, async AI request + response rendering, post-close side effects (trust / registry / persistence / autosave) |
...
- **Encounter-beat callers own scripted fallback.** `DialogueRequest.encounterBeat` is a story-beat AI request, not an ordinary first/return ambient greeting. `FallbackDialogueService` must not let generic unplayed human scripts (e.g. `camille_return`) intercept these calls before AI, and must rethrow AI failures so `GameScene.requestCamilleEncounterLines` can return `null` and let the authored `camille-encounter-beats.ts` fallback merge run. Otherwise a generic one-line ambient script can replace beat-specific spoken content and bypass the beat objective.
...
### System + scene-facade refactor (Commits A–D on `sit`)

- **Goal delivered:** `GameScene.ts` 4715 → 2183 LoC (~54% reduction) without a single save/registry/schema change and with every WORKING_MEMORY invariant below re-verified in tests after each commit. The scene is now a facade that owns Phaser lifecycle, tilemap + collision setup, input (keyboard + touch queues), camera (default zoom, peek zoom), save/load glue, and the `tryInteract()` outer shell; everything else polls from a dedicated system.
- **Shipped systems and their responsibilities:**
  - `SnatcherSystem` — nightly spawn / detect / capture lifecycle + colony-cat grab sweep. Still calls back into the scene for `loseLife`, `autoSave`, `triggerGameOver`, and camera fade; Phase 4.5 reveal-time witness gate (proximity + `hasLineOfSight`) preserved at the reveal site.
  - `ColonyDynamicsSystem` — dumping events, background roster spawn, and the single mutation owner for `StoryKeys.COLONY_COUNT` (seed / bump / decrement with floor `NAMED_AND_MAMMA_COUNT`). Exposes `onCatRemoved()` as the atomic decrement-plus-narrate entry point; `SnatcherSystem` is the sole caller (invoked after `removeColonyCat(cat)` bumps `CATS_SNATCHED`), so population bookkeeping stays co-owned by `ColonyDynamicsSystem` and `SnatcherSystem`.
  - `CamilleEncounterSystem` — ambient dawn/evening visits, Camille beats 1–5, paired narrator/spoken rendering via `playPairedBeat`, Beat-5 three-phase consent gate (predecision → 10 s window → journey), pickup tween. Pause-across-`onComplete` lifetime is caller-owned; `tryAcceptBeat5Decision()` is the only public call from `tryInteract()`.
  - `HumanPresenceSystem` — ambient human spawn/update, stationary-greet cap + anchor, scripted/AI greeting bubbles, the single `renderHumanBubble` surface (5.1a rule). Owns the `this.humans[]` roster and exposes `register/unregister` for Camille care-route and Snatcher entities — migrated atomically in Commit C so no mixed-ownership window ever existed.
  - `CollapseSystem` — rising/falling-edge detection, 3 s blackout timer, witness capture **before** teleport (WORKING_MEMORY "Position-dependent checks across a teleport"), witness-aware recovery narration. `tick(delta)` returns `"continue" | "blackout" | "game_over"` so the scene only translates outcomes into its existing control flow.
  - `CatDialogueController` — engagement gating, async AI request, response normalisation (hostile pose + positive emote is overridden to `hostile`), response rendering, post-close side effects (trust awards, registry flags, indicator reveals, `storeConversation` + `addNpcMemory`, autosave on first meet). Dialogue-chaining guard (`lastDialoguePartner` + `LAST_PARTNER_HOLD_MS = 1500`) cleared on range exit OR time expiry — logic lives in `refreshLastPartner(cat, dist, now)` and is called from the scene's NPC update loop.
  - `TerritorySystem` — existing claim API plus the flood-filled reachable-cell set moved off the scene. Scene passes its tilemap-aware `isBlocked` predicate to `initialiseExploration(...)`; `visitCell(tileX, tileY, mapWidth)` returns the packed key when the cell is reachable, else `null`, so the scene's `visitCurrentCell` is a two-liner.
- **Invariants re-verified per commit** (each commit ran `npx vitest run && npx tsc --noEmit && npx vite build && npm run verify:dist` before `git commit`):
  - Polling model (no new `EventEmitter` between systems).
  - Every system has a `shutdown()` chained from `GameScene.shutdown()`; edge flags (`wasCollapsed`, `lastDialoguePartner`, `playerInputFrozen`, stationary-greet counters) cleared on shutdown. The scene registers `this.events.once(Phaser.Scenes.Events.SHUTDOWN, this.shutdown, this)` in `create()`.
  - `COLONY_COUNT` mirror invariant — field + registry update in the same statement, single owner.
  - Beat pause/resume is caller-owned across `onComplete` (5.1b contract).
  - Phase 4.5 witness gate re-checked at the reveal site, not just the arming site, for snatcher first-sighting, dumping reveals, and Camille beats.
  - `snatcherSpawnChecked` ordering — still routed through `resolveSnatcherSpawnAction`.
  - Pre-teleport position snapshots preserved (snatcher capture, Beat-5 pickup, collapse recovery). `CollapseSystem.recoverFromCollapse` calls `scene.onCollapseTeleport(safeX, safeY)` which snapshots `previousPlayer{X,Y}` + re-fires `visitCurrentCell` so the post-teleport movement poll doesn't register a 1000+ px "step".
  - `tryInteract` routing order from 5.1b: Beat-5 acceptance → food sources → cat engagement → Mamma-Cat greeting fallback.
- **Public surface additions on `GameScene` (needed so systems can call back):** `pendingGameOverReason`, `syncTrustDisposition(catName)`, `addKnownCat(name)`, `narrateIfPerceivable(line, source?, radius?)`, `onCollapseTeleport(x, y)`, `startChapter6Sequence()`. Everything else stays internal; the scene is still the orchestrator. The `POSE_TO_EMOTE`, `POSITIVE_EMOTES`, and `CatDialoguePersistenceSnapshot` symbols now live in `CatDialogueController` — do not re-declare them on the scene.
- **Testing lesson (important).** `tests/utils/touchInputWiring.test.ts` uses `makeFrozenUpdateScene()` which builds a bare scene via `new GameScene()` and stubs in each system by hand. Every time a new system is wired into `update()`, the mock must stub it: the failing-before-green sequence across Commits B/C/D added `camille`, `humans`, `collapse`, `catDialogue`, and `territory` mocks in turn, plus a `map: { width: 100 }` stub once `visitCurrentCell` started reading `this.map.width` unconditionally. Rule: after any `update()` change, re-run `npx vitest run -- tests/utils/touchInputWiring.test.ts` first.
- **Commit E (deferred).** Slimming `processDialogueResponse` into a thin router that delegates per-event-string to `{system}.applyDialogueEvent(event, ctx)` is still a readability/cohesion win but not a blocker — the current switch inside `CatDialogueController.processResponse` is compact and covered by the scripted-event regression tests.
- **Content-bearing data moves verbatim.** Dialogue pools, narration strings, scripted greeting lines, and other authored content must be moved by targeted patch (or byte-level compare against `git show <prev-commit>:path`), never retyped or paraphrased. Commit B shipped with a silently regressed `camillePersonalLines` → `personalLines` dictionary (every named-cat greeting reworded, Blacky + Tiger also losing a line each) because the dictionary was re-keyed from memory instead of copied. There are no tests pinning story dialogue text, so content regressions pass every gate — the only defence is diff discipline at the point of the move.
- **In-flight LLM requests carry an AbortController keyed off the system's shutdown path.** Every system that awaits `FallbackDialogueService.getDialogue(...)` MUST (a) create an `AbortController` per call and stash it on the instance, (b) pass `{ signal: abort.signal }` to `getDialogue`, (c) re-check `abort.signal.aborted` after every `await`, (d) abort the stored controller from `resetTransient()` / `shutdown()`, and (e) detect `AbortError + signal.aborted` in the catch to suppress the normal error-path HUD/state cleanup (the scene is tearing down). Reference pairs: `HumanPresenceSystem.humanAiBubbleAbort` (shutdown line ~763) and `CatDialogueController.requestAbort` (resetTransient). `FallbackDialogueService` explicitly distinguishes caller-aborts from internal timeouts (`FallbackDialogueService.ts:42-67`) and rethrows caller-aborts so the caller's catch owns cleanup. Skipping any of the five steps leaks network round-trips past shutdown and, in any future mid-session reset, lets a stale continuation mutate scene/UI state.
- **`DialogueSystem.advance()` fires `onHide` BEFORE its captured `doneHook`.** Inside `advance()` the sequence on natural completion is `hide()` → `hooks.onHide()` → `doneHook()` — so any closure `completed` flag set inside the `onComplete` wrapper is still `false` when `onHide` runs. Don't use it to branch. If a paired beat kicks off irreversible side effects from `onStepShown(index)` — Beat-5 journey starts `runBeat5Pickup` on index 0, which fades Mamma, disables her body, and tweens Camille off-screen — then an early dismiss on the modal leaves `playerInputFrozen = true`, `ENCOUNTER_5_COMPLETE` unset, and no Chapter 6 transition: permanent soft-lock. Opt such beats into `playPairedBeat({ completeOnDismiss: true })` so the caller's `onComplete` runs from both paths, and keep the idempotency guard (`if (completed) return;` at the top of the `dialogue.show` onComplete wrapper) so the still-captured `doneHook` doesn't double-fire Chapter 6 / autoSave on natural completion.
...
- **GameScene size is now ~2200 lines (was 4715 before Commits A–D).** Extraction completed across four sequential commits on the `sit` branch: Commit A — `SnatcherSystem` + `ColonyDynamicsSystem`; Commit B — `CamilleEncounterSystem`; Commit C — `HumanPresenceSystem` (with atomic `register/unregister` migration for Camille + Snatcher callers); Commit D — `CatDialogueController` + `CollapseSystem` + `TerritorySystem` exploration fold-in. Intro cinematic + world-prop placement (~600 LoC) is deliberately still scene-owned. Optional Commit E (thin `processDialogueResponse` router that delegates per-event to each system's `applyDialogueEvent`) remains deferred. See the "System + scene-facade refactor (Commits A–D)" lesson below.
