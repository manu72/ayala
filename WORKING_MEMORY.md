...
- **Shipped systems and their responsibilities:**
  ...
  - `ColonyDynamicsSystem` — dumping events, background roster spawn, and the single mutation owner for `StoryKeys.COLONY_COUNT` (seed / bump / decrement with floor `NAMED_AND_MAMMA_COUNT`). Exposes `onCatSnatched(cat)` as the atomic decrement-plus-narrate entry point called from `SnatcherSystem`.
